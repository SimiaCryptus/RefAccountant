...
public abstract class ReferenceCountingBase implements ReferenceCounting {
  ...
  public static final boolean lifecycleDebug = true;
  private transient final UUID objectId = lifecycleDebug ? UUID.randomUUID() : jvmId;
  ...
  @Nullable
  private transient final StackTraceElement[] createdBy = lifecycleDebug ? Thread.currentThread().getStackTrace() : null;
  ...
  public void freeRef(ReferenceCounting obj) {
    ...
    synchronized (freeRefObjs) {
      if (lifecycleDebug) freeRefs.add(Thread.currentThread().getStackTrace());
